...
let jinxes = "https://cdn.discordapp.com/attachments/696695228496871436/955088191399034880/Jinx_rules.png"
...
async function ping_players(msg) {
  ...
  let txt = "**Game Just Ended**\n"
  for (var i = 0; i < next_game.length; i++) {
    txt += "<@" + next_game[i] + "> "
  }
  next_game = []
  await send_message(msg, txt)
}
...
client.on('messageCreate',
  async function(msg) {
    ...
    if (msg.content.trim().toLowerCase() === "*nospec") {
      ...
      if (msg.member.displayName.trim().charAt(0) != '!') {
      ...
      // if(msg.member.displayName.charAt(0) != '!')
      // {
      //     await msg.member.setNickname(`!${msg.member.displayName}`).catch(err => {await respond(msg, "```You must have an exclamation mark '!' before your name to be able to spectate!```"); return null;}) //{msg.reply("```You must have an exclamation mark '!' before your name to be able to spectate!```")})
      //     if(msg.member.displayName.charAt(0) != '!')
      //     {
      //         console.log("jkalsdfjkasdf")
      //         return null
      //     }
      // }
      if (msg.member.displayName.charAt(0) != '!' && msg.member.displayName.trim().substring(0, 4).toLowerCase() !== "(st)" && msg.member.displayName.trim().substring(0, 6).toLowerCase() !== "(cost)" && msg.member.displayName.trim().substring(0, 7).toLowerCase() !== "(co-st)") {
    ...
    else if (msg.content.trim().toLowerCase() === "*maintain") {
    ...
    else if (msg.content.trim().toLowerCase() === "*credits") {
    ...
    // }
      ...
      try {
        ...
        if (pc <= 4) {
          await respond(msg, "```There are currently not enough players for a game in the voice channels of this channel group\nMinimum: 5 for a teensy\nMinimum: 7 for a regular game```")
          return null
        }
        if (pc > 15) {
          await respond(msg, "```There are currently too many players for a game in the voice channels of this channel group\nMaximum: 15 regular players (+5 travelers)```")
          return null
        }
        if (pc + tc > 20) {
          await respond(msg, "```There are currently too many travelers for a game in the voice channels of this channel group\nMaximum for a game is 20 players total```")
          return null
        }
        var ind = pc - 5;
        res += "**>> The current composition should typically be:**\n";
        ...
        res += "*But remember to pay attention to roles that could effect the setup*"
        await respond(msg, res)
      } catch (Exception) {
        console.log("oh no")
      }
...
.setThumbnail("https://wiki.bloodontheclocktower.com/images/"+json[mr]["id"][0]+"/"+json[mr]["id"]+"/Icon_"+json[mr]["sn"]+".png")
    ...
    // }
      ...
      if (x.trim().toLowerCase().substring(0, 27).trim() !== "https://clocktower.online/#") {
        await respond(msg, "```Please provide a link to the grim in the command.```")
        // return null
      }
    ...
    else if (msg.content.trim().toLowerCase().indexOf("https://clocktower.online/#") > -1) {
      ...
      if (msg.member.displayName.trim().substring(0, 4).toLowerCase() !== "(st)" && msg.member.displayName.trim().substring(0, 6).toLowerCase() !== "(cost)" && msg.member.displayName.trim().substring(0, 7).toLowerCase() !== "(co-st)") {
        ...
        thelink = ""
        for (var i = msg.content.trim().toLowerCase().indexOf("https://clocktower.online/#"); i < msg.content.trim().length; i++) {
    ...
    if (msg.content.trim().toLowerCase().indexOf("https://clocktower.online/#") > -1 && !msg.author.bot) {
      // console.log("TEST1");
  ...
  if (oldState.channelId != null && newState.channelId != null && newState.channelId != oldState.channelId) {
    ...
    if (medic[newState.member.user.username] !== undefined && medic[newState.member.user.username] != -1)//(Object.keys(medic).indexOf(newState.member.user.username) != -1)
    {
      ...
      for (var i = 0; i < specs.length; i++) {
        ...
        {
          if (specs[i].displayName.charAt(0) != '!' && specs[i].displayName.trim().substring(0, 4).toLowerCase() !== "(st)" && specs[i].displayName.trim().substring(0, 6).toLowerCase() !== "(cost)" && specs[i].displayName.trim().substring(0, 7).toLowerCase() !== "(co-st)") {
            specs.splice(i, 1)
            i--
            continue
          }
...
client.on('rateLimit', async function(info) {
  ...
  responding = true;
  // client.destroy()
  // if(client.options.http.api === "https://discord.com/api") {
  //     client.options.http.api = "https://discordapp.com/api"
  // }
  // else {
...
try {
  ...
  console.log("GOOOOOOOOOOOOOOOOOOOOOOOOOOOOO")
  exec("kill 1", (error, stdout, stderr) => {
    if (error) {
      console.log(`error: ${error.message}`);
      return;
...
24) Remove previous tags: *p or *play
25) Add/Remove [AFK]:      *afk
26) Add/Remove [BRB]:      *brb
27) Remove AFK/BRB tags: *back or *here
28) Change nickname temporarily: *vote <name>
29) Back to original nickname:   *unvote
2A) Add/Remove [N] (Optional tag for new players): *n
\`\`\``
var helptxt2 =
  `\`\`\`
...
Extra Commands:
---------------
40) Check out recent changes to ShadowBOT: *changes or *changelog or *new
41) Tell a random joke:    *joke
42) Compliment someone <3: *compliment @<player>
43) Celebrate with style with a slow clap: *slowclap or *slow clap
...
26) Add/Remove [BRB]:      *brb
27) Remove AFK/BRB tags: *back or *here
28) Change nickname (useful for boomdandy): *vote <name>
29) Go back to original nickname:   *unvote
2A) Add/Remove [N] (Optional tag for new players): *n
...
var changes =
  `**Latest Changes:**\n- Fixed a few minor issues with the search algorithm of \`*role\` and \`*fable\`\n- Made improvements to counteract the rate-limits problem.`;
